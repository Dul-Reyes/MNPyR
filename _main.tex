...
\hypersetup{
            ...
            pdfauthor={Sofía Villers Gómez; Carlos Fernando Vásquez Guerra; Luis Angel Ramirez Teodoro},
            pdfborder={0 0 0},
            breaklinks=true}
...
\tightlist
\item
  Un programa ecológico sobre la contaminación de un río tomó 5 muestras
  de agua de diferentes lugares de un río antes y después de dos años,
  obteniéndose los siguientes resultados. Los números representan la
  media de la contaminación, donde medidas grandes indican alta
  contaminación.
\end{enumerate}
...
Se está interesado en saber si el programa de rehabilitación ecológica
ha tenido efecto en la reducción de la contaminación. Use
\(\alpha =1%\).
...
\tightlist
\item
  Se presentan a continuación los tipos de cambio MXN-USD de los últimos
  30 días.
\end{enumerate}

24.48 , 24.07 , 23.66 , 23.70 , 23.93 , 24.34 , 24.22 , 23.87 , 23.96 ,
24.05 , 23.78 , 23.74 , 23.21 , 22.88 , 22.74 , 22.73 , 22.58 , 22.21 ,
22.33 , 22.21 , 22.17 , 22.21 , 22.04 , 21.79 , 21.76 , 21.91 , 21.58 ,
21.60 , 21.50 , 21.91

Se desea corroborar la aseveración del presidente respecto a que el peso
mexicano esta recuperando fuerza. Use \(\alpha =10%\).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1=}\KeywordTok{lm}\NormalTok{(Loblolly}\OperatorTok{$}\NormalTok{height}\OperatorTok{~}\NormalTok{Loblolly}\OperatorTok{$}\NormalTok{age)}
...
\begin{center}\includegraphics{_main_files/figure-latex/unnamed-chunk-84-2} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data}\NormalTok{(}\StringTok{"trees"}\NormalTok{)}
\KeywordTok{library}\NormalTok{(}\StringTok{"scatterplot3d"}\NormalTok{)}
\NormalTok{s3d <-}\StringTok{ }\KeywordTok{scatterplot3d}\NormalTok{(trees, }\DataTypeTok{type =} \StringTok{"h"}\NormalTok{, }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{,}
    \DataTypeTok{angle=}\DecValTok{55}\NormalTok{, }\DataTypeTok{pch =} \DecValTok{16}\NormalTok{, }\DataTypeTok{xlab =} \StringTok{"Diametro del arbol"}\NormalTok{,}\DataTypeTok{ylab =} \StringTok{"Altura del arbol"}\NormalTok{, }\DataTypeTok{zlab=}\StringTok{"Volumen"}\NormalTok{)}
\NormalTok{my.lm <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(trees}\OperatorTok{$}\NormalTok{Volume }\OperatorTok{~}\StringTok{ }\NormalTok{trees}\OperatorTok{$}\NormalTok{Girth }\OperatorTok{+}\StringTok{ }\NormalTok{trees}\OperatorTok{$}\NormalTok{Height)}
\NormalTok{s3d}\OperatorTok{$}\KeywordTok{plane3d}\NormalTok{(my.lm)}
\end{Highlighting}
\end{Shaded}
